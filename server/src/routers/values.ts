      ...
      if (input.interviewId) {
        ...
        if (rd) {
          ...
          if (rd.status !== 'completed') {
            ...
            // No "interview complete" notification is sent here (removed per Jade,
            // 2026-08-05 — it was noise). The round's interviewer still receives
            // their AI debrief separately via processInterview
            // (emailInterviewFeedbackInterviewer), and the NEXT round's interviewer
            // is briefed just below via sendNextRoundPrep.
          }
