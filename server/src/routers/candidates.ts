...
import { computeAndStoreScreen } from '../services/resumeScreen.js';
...
export const candidatesRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Placeholder mode sends the assessment with the application confirmation,
      // so the candidate is effectively in assessment from the moment they apply —
      // move them into the Assessment stage (Applied becomes a brief pass-through).
      // Real-Criteria mode leaves them in Applied until the invite goes out later.
      if (!isCriteriaConfigured() && candidate.currentStage === 'Applied') {
        await ctx.db.update(candidates).set({ currentStage: 'Assessment', updatedAt: new Date() }).where(eq(candidates.id, candidate.id));
        await ctx.db.insert(candidateStageHistory).values({
          candidateId: candidate.id, fromStage: 'Applied', toStage: 'Assessment', changedBy: null,
          reason: 'Assessment sent with the application confirmation',
        });
      }
  ...
  runScreen: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const screen = await computeAndStoreScreen(ctx.db, input.id, { needsSponsorship: input.needsSponsorship, resumeText });
      if (!screen) throw new TRPCError({ code: 'BAD_REQUEST', message: 'No resume on file for this candidate to screen.' });
      const { recommendation, decision, reason, composite, requirements, niceToHaves, skills, eppMatch, companyValuesMatch, eppScans } = screen;
