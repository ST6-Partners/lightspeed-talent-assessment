...
const EMPTY = { name: '', level: '', department: '' };
...
export default function Titles() {
  ...
  const { data: rows, refetch } = trpc.titles.list.useQuery();
  const close = () => { setShowForm(false); setEditingId(null); setForm({ ...EMPTY }); };
  const createM = trpc.titles.create.useMutation({ onSuccess: () => { refetch(); close(); } });
  const updateM = trpc.titles.update.useMutation({ onSuccess: () => { refetch(); close(); } });
  const deleteM = trpc.titles.delete.useMutation({ onSuccess: () => refetch() });

  const startEdit = (r: any) => { setEditingId(r.id); setForm({ name: r.name ?? '', level: r.level ?? '', department: r.department ?? '' }); setShowForm(true); };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Standard job titles and levels</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-lg border border-gray-200 p-5 mb-6">
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-semibold text-gray-700">{editingId ? 'Edit Title' : 'New Title'}</span>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        {!rows || rows.length === 0 ? (
          <div className="p-8 text-center text-gray-400 text-sm">No titles yet.</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 text-left text-xs font-medium text-gray-500 uppercase">
                <th className="px-4 py-3">Title</th><th className="px-4 py-3">Level</th><th className="px-4 py-3">Department</th><th className="px-4 py-3 w-20"></th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50 text-sm">
                  ...
                  <td className="px-4 py-3 text-gray-500">{r.level || '—'}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => { if (confirm(`Delete title "${r.name}"?`)) deleteM.mutate({ id: r.id }); }} className="p-1 text-gray-400 hover:text-red-600" title="Delete"><Trash2 size={15} /></button>
                    </div>
                  </td>
                </tr>
              ))}
