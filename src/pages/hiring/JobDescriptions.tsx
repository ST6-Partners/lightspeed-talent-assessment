          ...
          <table className="w-full">
            ...
            <tbody>
              {jobDescriptions.map((jd) => (
                <tr key={jd.id} className={`border-b border-gray-50 text-sm ${(jd as any).autoGenerated ? 'bg-amber-50 hover:bg-amber-100' : 'hover:bg-gray-50'}`}>
                  <td className="px-4 py-3 font-medium text-gray-900">
                    {jd.jobTitle}
                    {(jd as any).autoGenerated && <span className="ml-2 inline-flex px-1.5 py-0.5 text-[10px] rounded-full bg-amber-200 text-amber-800 align-middle">NEW · auto-generated, review</span>}
